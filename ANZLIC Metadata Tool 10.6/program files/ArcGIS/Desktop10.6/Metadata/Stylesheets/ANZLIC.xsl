...
-->
  ...
  <!-- Esri Australia 2013 -->
 <xsl:variable name="ANZLIC" select="/metadata/idinfo//*" />
...
-->  
  ...
  <xsl:if test="$flowdirection = 'RTL'">
	...
	<xsl:if test="$arcgis1[text()] or $arcgis2[text()] or $arcgis3[text()]">
		<h2 class="iso head"><a href="#arcgisMetadata" onclick="hideShow('arcgisMetadata')">
			<xsl:attribute name="title">
				<xsl:value-of select="res:str('idTopArcGISTooltip')"/>
			</xsl:attribute>
	...
	<xsl:if test="not($arcgis1[text()] or $arcgis2[text()] or $arcgis3[text()])">
